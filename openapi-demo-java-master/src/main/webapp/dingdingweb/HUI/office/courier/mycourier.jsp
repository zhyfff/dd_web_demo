...
function getMyCourier(){
	$.ajax({
		url:'${basePath}Office/Courier/getMyCourier?searchkey='+$("#userName").text()+'&pageNum=1',
		type:'GET',
		dataType:'text',
		success:function (result){
			...
			for (var i = 0; i < blist.length; i++) {
				...
				var imgurl='';
				if (s=='已领取需确认') {
					imgurl='dingdingweb/HUI/img/typelist/lqz.png';
				}else if (s=='未领取') {
					imgurl='dingdingweb/HUI/img/typelist/wlq.png';
				}else if (i==0) {
					imgurl='dingdingweb/HUI/img/typelist/new.png';
				}else{
					imgurl='dingdingweb/HUI/img/typelist/ylq.png';
				}
				
				var div = document.createElement('div');
				 div.innerHTML ='<div class="hui-accordion" onclick="auditY(this)">'+
					'<div class="hui-accordion-title" style="background-image:url('+imgurl+');background-repeat:no-repeat;background-position:70% 20%;background-size:48px 48px;">'+recipients+'-'+express+'</div>'+
					'<div class="hui-accordion-content hui-list">'+
						'<form action="${basePath}Office/SaveMyCourier" method="post" id="saveMyCourier">'+
...
function auditY(target){
	var y = $(target).children(".hui-accordion-content").children("#saveMyCourier").children(".hui-form-items").find("#s").attr("value");
//		alert(y);
...
function nextPage(){
	$.ajax({
		url:'${basePath}Office/Courier/getMyCourier?searchkey='+$("#userName").text()+'&pageNum='+page,
		type:'get',
		dataType:'text',
		success:function(result){
			var re = JSON.parse(JSON.stringify(result));
