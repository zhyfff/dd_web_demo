...
var icons = ["icon-gouwuche","icon-chaxun","icon-haocaiguanli"];
var html  = '';
html += '<li>'+
	'<div class="hui-speed-dial-icons" onclick="apply()">'+
        '<span class="icon iconfont '+icons[0]+'"></span>'+
    '</div>'+
    '<div class="hui-speed-dial-text">采购</div>';
	'</li>';
...
html += '<li>'+
	'<div class="hui-speed-dial-icons" onclick="shop()">'+
	    '<span class="icon iconfont '+icons[2]+'"></span>'+
	'</div>'+
	'<div class="hui-speed-dial-text">耗材仓库</div>';
	'</li>';
hui('.hui-speed-dial ul').html(html);
...
function apply(){
	...
	if (info == "") {
		...
		window.location = 'ITServices/consuable_pur/apply?info='
...
function shop(){
	...
	if (info == "") {
		...
		window.location = 'ITServices/consuable_pur/shop?info='
				+ encodeURIComponent(info)
	}	
}

function search(){
	window.location='ITServices/consuable_pur/tosearch';
}
