...
function getAllStampApply(){
	$.ajax({
		url:'${basePath}Office/StampCertificate/getAllStampApply?pageNum=1',
		type:'GET',
		dataType:'text',
				...
				var div = document.createElement('div');
				 div.innerHTML ='<div class="hui-accordion" onclick="auditY(this)">'+
					...
					'<div class="hui-accordion-content hui-list">'+
							'<form action="${basePath}Office/StampCertificate/SureReturnStampApply" method="post" id="Return">'+
...
//		alert(y);
	if (y.indexOf("已归还请确认")==-1) {
		$(target).children(".hui-accordion-content").children("#Return").children("#never").hide();
	}
}
</script>
<title>确认印章证照归还</title>
</head>
<body>
	<header class="header">
...
function nextPage(){
	$.ajax({
		url:'${basePath}Office/StampCertificate/getAllStampApply?pageNum='+page,
		type:'get',
		dataType:'text',
		success:function(result){
			var re = JSON.parse(JSON.stringify(result));
