...
function getNeedAudit(){
	$.ajax({
		url:'${basePath}Office/StampCertificate/getNeedAuditApplyStamp?audit_name='+$("#userName").text()+'&pageNum=1',
		type:'GET',
		dataType:'text',
				...
				var div = document.createElement('div');
				 div.innerHTML ='<div class="hui-accordion" onclick="auditY(this)">'+
					...
					'<div class="hui-accordion-content hui-list">'+
							'<form action="${basePath}Office/StampCertificate/toManger" method="post" id="tomanger">'+
...
//		alert(y);
	if (y.indexOf("请审批")==-1) {
		$(target).children(".hui-accordion-content").children("#tomanger").children("#never").hide();
	}
}
</script>
<title>证章借用审核</title>
...
function nextPage(){
	$.ajax({
		url:'${basePath}Office/StampCertificate/getNeedAuditApplyStamp?audit_name='+$("#userName").text()+'&pageNum='+page,
		type:'get',
		dataType:'text',
		success:function(result){
			var re = JSON.parse(JSON.stringify(result));
