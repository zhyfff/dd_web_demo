...
function getAuditedApply(){
	$.ajax({
		url:'${basePath}Office/StampCertificate/getAuditedApplyStamp?manager='+$("#userName").text()+'&pageNum=1',
		type:'GET',
		dataType:'text',
				...
				var div = document.createElement('div');
				 div.innerHTML ='<div class="hui-accordion" onclick="auditY(this)">'+
					...
					'<div class="hui-accordion-content hui-list">'+
							'<form action="${basePath}Office/StampCertificate/toIssue" method="post" id="toIssue">'+
...
function nextPage(){
	$.ajax({
		url:'${basePath}Office/StampCertificate/getAuditedApplyStamp?manager='+$("#userName").text()+'&pageNum='+page,
		type:'get',
		dataType:'text',
		success:function(result){
			var re = JSON.parse(JSON.stringify(result));
