...
function getRelq(){
	$.ajax({
		url:'${basePath}Office/Courier/getRelq?pageNum=1',
		type:'GET',
		dataType:'text',
				...
				var div = document.createElement('div');
				 div.innerHTML ='<div class="hui-accordion" onclick="auditY(this)">'+
					...
					'<div class="hui-accordion-content hui-list">'+
						'<form action="${basePath}Office/SaveRelq" method="post" id="saveMyCourier">'+
...
function nextPage(){
	$.ajax({
		url:'${basePath}Office/Courier/getRelq?pageNum='+page,
		type:'get',
		dataType:'text',
		success:function(result){
			var re = JSON.parse(JSON.stringify(result));
